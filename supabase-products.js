...
(function () {
  ...
  const RELOAD_GUARD_KEY = 'muv_supabase_auto_reload_done';
  function canAutoReload() {
    // Nunca recarrega mais de uma vez por aba: evita loop infinito de reload
    // caso o catálogo remoto e o local nunca fiquem "iguais" (ex.: instabilidade
    // de dados). Uma vez recarregado, a página segue funcionando normalmente
    // mesmo que a sincronização em segundo plano detecte outra diferença.
    ...
    if (!remote.length && local.length && document.getElementById('productForm')) {
      ...
      if (hasLocalImages && !sessionStorage.getItem('muv_catalog_migration_prompted')) {
        ...
        if (ok) {
          ...
          if (forceReload && canAutoReload()) location.reload();
          return migrated;
        }
      }
    }
